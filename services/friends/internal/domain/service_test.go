...
func (r *repositoryStub) RunInTransaction(ctx context.Context, fn func(txCtx context.Context) error) error {
	r.transactionCalls++
	return fn(ctx)
}
...
func TestAcceptFriendRequest(t *testing.T) {
	...
	if err := service.AcceptFriendRequest(context.Background(), "user-2", "request-1"); err != nil {
		t.Fatalf("AcceptFriendRequest returned error: %v", err)
	}

	if repo.transactionCalls != 1 {
		t.Fatalf("expected 1 transaction call, got %d", repo.transactionCalls)
	}
...
func TestDeclineFriendRequest(t *testing.T) {
	...
	if err := service.DeclineFriendRequest(context.Background(), "user-2", "request-1"); err != nil {
...
func TestAcceptFriendRequestRejectsUnauthorizedActor(t *testing.T) {
	...
	err := service.AcceptFriendRequest(context.Background(), "user-3", "request-1")
...
func TestDeclineFriendRequestRejectsUnauthorizedActor(t *testing.T) {
	...
	err := service.DeclineFriendRequest(context.Background(), "user-3", "request-1")
	if !errors.Is(err, ErrUnauthorizedFriendRequest) {
		t.Fatalf("expected ErrUnauthorizedFriendRequest, got %v", err)
	}
}
