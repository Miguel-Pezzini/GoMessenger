...
func (s *Service) AcceptFriendRequest(ctx context.Context, actorID, requestID string) (FriendRequest, error) {
	...
	if requestID == "" {
		return FriendRequest{}, ErrInvalidRequestID
	}
	...
	exists, err := s.repo.FriendshipExists(ctx, request.SenderID, request.ReceiverID)
	if err != nil {
		return FriendRequest{}, err
	}

	if !exists {
		if err := s.repo.CreateFriendships(ctx, request.SenderID, request.ReceiverID, s.now()); err != nil {
			return FriendRequest{}, err
		}
	}

	err = s.repo.DeleteFriendRequestByID(ctx, request.ID)
	if err != nil {
		if errors.Is(err, mongo.ErrNoDocuments) {
			return FriendRequest{}, ErrFriendRequestNotFound
		}
...
func (s *Service) DeclineFriendRequest(ctx context.Context, actorID, requestID string) (FriendRequest, error) {
	actorID = strings.TrimSpace(actorID)
	requestID = strings.TrimSpace(requestID)
